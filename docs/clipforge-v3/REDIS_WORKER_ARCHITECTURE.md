...
- Worker revision validated: `clipforge-tools-worker-00007-zvn`
- Worker instances: min `1`, max `1`
- Worker concurrency: `1`
- Worker CPU: no throttling while instance is allocated
- Queue: `clipforge`
- Web candidate revision: `clipforge-tools-pg-worker-ready`
- Web candidate traffic: `0%`, tag `pg-candidate`
- Production traffic: still `100%` on SQLite revision `clipforge-tools-00104-hwm`
- Queue smoke: passed using Cloud Run Job `clipforge-queue-smoke`.
- Restart/idempotency code path: covered by V3 real-provider alpha tests with mock provider; no real Ark/Seedance call was made.
- Current blocker before traffic cutover: a fresh maintenance-window SQLite snapshot/import is still required because production traffic has remained on SQLite after the candidate import.
- Do not enable paid generation or switch traffic until the final fresh snapshot/import and human cutover approval are complete.
