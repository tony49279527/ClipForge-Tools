...
class FakeR2Client:
    ...
    def head_object(self, *, Bucket, Key):
        ...
        return {"ContentLength": len(self.objects[Key]["body"]), "ContentType": self.objects[Key].get("content_type")}
    ...
    def put_object(self, *, Bucket, Key, Body, ContentType, CacheControl):
        ...
        self.objects[Key] = {"body": body, "content_type": ContentType, "cache_control": CacheControl}
...
def _r2_storage(fake_client: FakeR2Client):
...
def test_r2_endpoint_can_be_built_from_account_id(monkeypatch):
    storage = importlib.import_module("clipforge_v3.services.storage_service")
...
def test_r2_save_file_uploads_with_content_type_and_reuses_existing_object(tmp_path):
    fake = FakeR2Client()
    r2 = _r2_storage(fake)
    video = tmp_path / "video.mp4"
    video.write_bytes(b"video")
    first = r2.save_file(project_id=1, source_path=video, object_key="projects/1/shots/1/submissions/1/video.mp4", content_type="video/mp4")
    second = r2.save_file(project_id=1, source_path=video, object_key=first["object_key"], content_type="video/mp4")
    assert first["object_key"] == second["object_key"]
    assert first["size_bytes"] == 5
    assert fake.put_calls == [
        {
            "Bucket": "clipforge-v3-test",
...
def test_r2_presigned_video_url_is_dynamic_and_clamped():
    fake = FakeR2Client()
    r2 = _r2_storage(fake)
